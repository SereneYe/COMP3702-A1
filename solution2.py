...
class Solver:
    ...
    def solve_ucs(self):
        ...
        print('Nodes visited/reached:', len(visited))
        print('Nodes expanded:', len(visited))  # visited nodes are expanded nodes
        print('Nodes on the frontier when the search terminates:', len(frontier))
...
if __name__ == "__main__":
    ...
    environment = Environment.from_file('data/1.txt')
    loop_counter = LoopCounter()  # Initiate an instance of LoopCounter

    solver = Solver(environment, loop_counter)
    path = solver.solve_ucs()  # Perform UCS
